...
import { fetchDailySummary } from '../api/summaryApi';
...
const DailySummaryCard = ({ date, sector, lastRefreshed }) => {
    ...
    return (
      <div style={styles.container}>
        <div style={styles.loadingContainer}>
          ...
          <span style={styles.loadingText}>Generating AI Market Summary...</span>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return null; // Fail silently or could show a small error state
  }

  if (!summary) {
    return null; // No summary available for this date
  }

  const sectorsList = summary.sectors ? summary.sectors.split(',').map(s => s.trim()) : [];
      ...
      <div style={styles.header}>
        <div style={styles.headerLeft}>
          ...
          <h2 style={styles.title}>
            Market Summary {summary.date_val && `(${summary.date_val})`}
          </h2>
        </div>
      ...
      <div style={styles.content}>
        ...
        {summary.events && (
          <div style={styles.eventsContainer}>
            ...
            <div style={styles.markdownWrapper}>
              <ReactMarkdown>{summary.events}</ReactMarkdown>
            </div>
          </div>
        )}
        ...
        {summary.reasoning && (
          <div style={styles.reasoningContainer}>
            ...
            <p style={styles.reasoningText}>{summary.reasoning}</p>
          </div>
        )}
      </div>
        ...
        <div style={styles.footer}>
          ...
          <div style={styles.sectorsWrapper}>
            {sectorsList.map((sector, idx) => (
              <SectorTag key={idx} sector={sector} />
            ))}
          </div>
        </div>
      )}
